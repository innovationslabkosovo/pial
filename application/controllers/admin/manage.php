...
class Manage_Controller extends Admin_Controller
{
	...
	public function index()
	{
		...
		if ($_POST)
		{
			...
			$category_data = arr::extract($post_data, 'parent_id',
				'category_title', 'category_description', 'category_color', 'child_id', 'category_position');
			...
			$secondary_data = arr::extract($post_data, 'category_image',
				'category_title_lang','category_description_lang', 'action', 'child_id', 'category_position');
			...
			if ($post->action == 'a')
			{
				$post->add_rules('category_image', 'upload::valid',
					'upload::type[gif,jpg,png,pdf,doc]', 'upload::size[50K]');
			...
			if ($post->action == 'a')
			{
				...
				if ($category->validate($category_data) AND $post->validate(FALSE))
				{
                    ...
                    if ($_POST['category_position'] == 0)
                    {
                        ...
                        $category->category_position = $category_data['category_position'];
                    }

                    //echo $_POST['child_id'];
                    //$category->child_id = $_POST['child_id'];
        ...
        $childrens_array = ORM::factory('category')
        					->where('parent_id', $parent_id)
				            ->where('child_id', '0')
				            ->select_list('category_position', 'category_title');

		// add none to the list
		$parents_array[0] = "--- Top Level Category ---";
		$childrens_array[0] = "--- Children Level Category ---";


		// Put "--- Top Level Category ---" at the top of the list
		ksort($parents_array);
		ksort($childrens_array);
		...
		$this->template->content->childrens_array = $childrens_array;
    ...
    /**
    * Get childrens of children category
    */
    public function get_childrens_array($parent_id = FALSE) {
        header('content-type: application/json');
        $this->auto_render=false;
        $this->template->content = new View('admin/manage/get_childrens_array');
        ...
        echo $this->template->content->childrens_array = json_encode($childrens_array);
    }
}
