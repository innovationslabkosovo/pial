	...
	public function edit($id = FALSE, $saved = FALSE)
	{
		...
		if ($_POST)
		{
			...
			if (reports::validate($post))
			{
				...
				// STEP 2b: Record Approval/Verification Actionjs
			...
			if (Incident_Model::is_valid_incident($id, FALSE))
			{
				...
				if ($incident->loaded == TRUE)
				{
					...
					$incident_arr = array(
						...
						'custom_field' => customforms::get_custom_form_fields($id, '', TRUE, 'submit'),
		...
		$custom_field_mismatch = customforms::get_edit_mismatch('');
		...
		$disp_custom_fields = customforms::get_custom_form_fields($id, 2, FALSE, "view");
		...
		$disp_custom_fields = customforms::get_custom_form_fields($id, 3, FALSE, "view");
