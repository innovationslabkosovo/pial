...
class Json_Controller extends Template_Controller {
	...
	protected function clusters_geojson($incidents, $category_id, $color, $icon)
	{
		...
		*/
        ...
        if ($zoomLevel > 11) {
            // Pass geometry points to standard markers json
            $json_features = array_merge($json_features, $this->markers_geojson($singles, $category_id, $color, $icon, TRUE));
        }
        else
...
//				$fillcolor = ($item->geometry_color) ? $item->geometry_color : "ffcc66";
				$fillcolor = ($item->geometry_color) ? $item->geometry_color : "f24163";

//				$strokecolor = ($item->geometry_color) ? $item->geometry_color : "CC0000";
				$strokecolor = ($item->geometry_color) ? $item->geometry_color : "590514";
