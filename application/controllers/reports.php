...
class Reports_Controller extends Main_Controller {
	...
	{
        // Redirect to home page, becuase we have disabled this form of submit
      // Only admin can submit data
		...
		// Retrieve Custom Form Fields Structure for Albanian
		$disp_custom_fields = customforms::get_custom_form_fields($id, 2, FALSE);
		$this->template->content->custom_forms_sq = new View('reports/submit_custom_forms');
		$this->template->content->custom_forms_sq->disp_custom_fields = $disp_custom_fields;
		$this->template->content->custom_forms_sq->form = $form;

		// Retrieve Custom Form Fields Structure for English
		$disp_custom_fields = customforms::get_custom_form_fields($id, 3, FALSE);
		$this->template->content->custom_forms_sr = new View('reports/submit_custom_forms');
		$this->template->content->custom_forms_sr->disp_custom_fields = $disp_custom_fields;
		$this->template->content->custom_forms_sr->form = $form;
	...
	public function view($id = FALSE)
	{
		...
		$c_lang = Kohana::config('locale.language.0');
		switch ($c_lang) {
			case 'sr':
				$l_lang = 3;
				break;
		...
		$form_field_names = customforms::get_custom_form_fields($id, $l_lang, FALSE, "view");
