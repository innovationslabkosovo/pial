...
<style type="text/css">
    #sub-child {
        background: url(/ushahidi/media/img/treeview_dir_line.png), url(/ushahidi/media/img/treeview_dir_line_.png);
        padding-left: 33px;
        background-repeat: no-repeat;
        background-position: 0px 0px, 0px -10px !important;
    }
    .last {
...
class category_Core {
    ...
    public static function form_tree($form_field, array $selected_categories = array(), $columns = 1, $enable_parents = FALSE, $show_hidden = FALSE)
    {
        ...
        foreach ($category_data as $category)
        {
            ...
            {
                $html .= "\t\t<ul>";
                foreach ($category['children'] as $child => $sub_child)
                {
                    // Check for sub-children
                    $sub_id = ($sub_child['child_id'] > 0) ? "sub-child" : "parent-category";
                    $html .= "\n\t\t\t".'<li id="'.$sub_id.'" title="'.$sub_child['category_description'].'">'."\n";
                    $html .= category::display_category_checkbox($sub_child, $selected_categories, $form_field, $enable_parents);
                    $html .= "\n\t\t\t".'</li>'."\r\n";
                }
                $html .= "\t\t".'</ul>'."\r\n";
            }
    ...
    public static function get_category_tree_data($count = FALSE, $include_hidden = FALSE)
    {
        ...
        if ($count)
        {
            $sql = "SELECT c.id, c.parent_id, c.child_id, c.category_title, c.category_color, c.category_image, c.category_image_thumb, COUNT(i.id) report_count "
        ...
        {
            $sql = "SELECT c.id, c.parent_id, c.child_id, c.category_title, c.category_color, c.category_image, c.category_image_thumb "
                . "FROM ".$table_prefix."category c "
                . "LEFT JOIN ".$table_prefix."category c_parent ON (c.parent_id = c_parent.id) "
                . "WHERE 1=1 "
                . (!$include_hidden ? "AND c.category_visible = 1 " : "")
    ...
    private static function _generate_treeview_html($category_data)
    {
        ...
        foreach ($category_data as $id => $category)
        {
            ...
            if ($category['child_id'] > 0)
            {
                $category_class = ($category['child_id'] > 0)? " class=\"report-listing-category-sub-child\"" : "";
            }
            else
            {
                $category_class = ($category['parent_id'] > 0)? " class=\"report-listing-category-child\"" : "";
