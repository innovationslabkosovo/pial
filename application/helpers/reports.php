...
class reports_Core {
	...
	public static function validate(array & $post)
	{
		...
		$post->add_rules('incident_photo', 'upload::valid', 'upload::type[gif,jpg,png,jpeg,pdf]', 'upload::size[2M]');
	...
	public static function save_location($post, $location)
	{
        ...
        $url = isset($uri[5]) ? $url = $uri[5] : false;

        // Edit Mode, but there is no new location
        if($location->loaded && empty($post->clear_map)) {
            $latitude = $location->latitude;
            $longitude = $location->longitude;
        }
        // Add New location with geometries or Edit location with new geometries
        else if ((!$location->loaded && empty($post->geometry) === FALSE) || ($location->loaded && empty($post->clear_map) === FALSE)) {
            $geom = explode(":", $post->geometry[0]);
            $point = explode(',', $geom[1]);
            $dets = explode(' ', $point[0]);
            $latitude = substr($dets[1], 0,-2);
            $longitude = substr($dets[0], 7);
        }
        // Add only one location
        else if (!$location->loaded && empty($post->geometry)) {
	...
	public static function save_report_geometry($post, $incident)
	{
		...
		if (isset($post->geometry)) 
		{
			...
			$sql = "INSERT INTO ".Kohana::config('database.default.table_prefix')."geometry "
                ...
                $uri = explode("/",$_SERVER['REQUEST_URI']);
                $url = isset($uri[5]) ? $url = $uri[5] : false;
                $query = "SELECT Count(*) as c FROM `geometry` WHERE incident_id = :incident";
            if ($url && empty($post->clear_map)) {
                ...
                foreach(array_slice($post->geometry, 1) as $item) {
                    if ( ! empty($item))
                    {
                        //Decode JSON
                        $item = json_decode($item);
