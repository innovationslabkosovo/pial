...
?>
		...
		jQuery(window).load(function() {
			...
			// Active panels in here for the map
			panel.activateControl(panel.controls[1]);
			drag.activate();
        ...
        $(document).ready(function() {
            $(".category-column").children().on("click", function(){
                console.log($(this));
            });
        });
		...
		$('.showForm').live('click',function(){
			$(this).removeClass('showForm')
				   .addClass('hideForm')
				   .html('-');
			$('.serbian_form').css('display', 'block');
		});

		$('.hideForm').live('click',function(){
			$(this).removeClass('hideForm')
				   .addClass('showForm')
				   .html('+');
			$('.serbian_form').css('display', 'none');
		});
