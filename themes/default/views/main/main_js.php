...
jQuery(function() {
	...
	$("#select_city").change(function() {
		...
		if ( lonlat[0] && lonlat[1] ) {
			map._olMap.setCenter(new OpenLayers.LonLat(lonlat[0],lonlat[1]).transform(fromProjection,toProjection), 13);
		}
	});

    // Sub-child Category Switch Action
    $("div.hide li > a").click(function(e) {
        var cat = this.className.substring(4);
        var parentCategory = $(this).parent().attr("class");
        if (parentCategory == undefined){
            if ( $(this).attr("class").split(" ")[1] != "active") {
                $("[id^='ch_']").attr("style", "display: none !important");
            }
        }
        $("li #ch_"+cat).each(function() {
            $(this).attr("style", "display: block !important");
        });
    });

    $("#circle_color").attr("fill", "#550ec0").attr("stroke", "#550ec0");

	// Category Switch Action
	$("ul#category_switch li > a").click(function(e) {

        var categoryColor = $(this).children(":first").attr("style").split(":")[1];
        var circleColor = $("#circle_color");

        if ($(this).attr("class") != "active") {
            $(circleColor).attr("fill", categoryColor)
